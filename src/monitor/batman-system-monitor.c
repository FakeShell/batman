...
get_system_info(int x86)
{
    ...
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            if (dir->d_type == DT_DIR &&
                ...
                printf("wlroots screen status for UID %s: %s",
                       dir->d_name, result == 1 ? "yes\n" : "no\n");
                break;
            }
        }
        closedir(d);
    ...
    for (int i = first_core; i <= last_core; ++i) {
        ...
        snprintf(path, sizeof(path),
                 "/sys/devices/system/cpu/cpu%d/cpufreq/scaling_available_governors", i);

        if ((file = fopen(path, "r"))) {
            if (fgets(buf, sizeof(buf), file)) {
                strtok(buf, "\n");
                snprintf(output_scaling + strlen(output_scaling),
                         sizeof(output_scaling) - strlen(output_scaling),
                         "scaling_available_governors=\"%s\" ", buf);
            }
            fclose(file);
        }
        ...
        snprintf(path, sizeof(path),
                 "/sys/devices/system/cpu/cpu%d/cpufreq/%s", i, cpufreq_node);

        if ((file = fopen(path, "r"))) {
            if (fgets(buf, sizeof(buf), file)) {
                strtok(buf, "\n");
                snprintf(output_current + strlen(output_current),
                         sizeof(output_current) - strlen(output_current),
                         "cur_freq=%s ", buf);
            }
            fclose(file);
        }
        ...
        char min_freq[128] = "", max_freq[128] = "";
        char scaling_min[128] = "", scaling_max[128] = "";
        ...
        printf("cpufreq: cpuinfo_min_freq=%s cpuinfo_max_freq=%s "
               "scaling_min_freq=%s scaling_max_freq=%s\n",
    ...
    for (size_t i = 0; i < n_paths; ++i) {
        ...
        char node_name[1024];
        strncpy(node_name, get_node_name(paths[i]), sizeof(node_name) - 1);
        ...
        while (fgets(buf, sizeof(buf), file)) {
            ...
            strncat(combined, buf, sizeof(combined) - strlen(combined) - 1);
            first = 0;
        }
